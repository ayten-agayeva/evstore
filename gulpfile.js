...
function copy_js(){
	return src(['src/js/lang/**/*']).pipe(dest('dist/js/lang'));
}
...
function html(){
	return src('src/*.html') // run the Twig template parser on all .html files in the "src" directory
		...
		.pipe(dest('dist'));// output the rendered HTML files to the "dist" directory
}
function watch_file(){
	...
	watch('src/*.html', html);
	watch('src/images/**/*', series(function(){ return del(['dist/images/**/*']); }, copy_images));
}
function serve(){
